...
```json
{
  ...
  "aabDeployPath": "\\\\sdrlazneuiis01d.corp.local\\sacred\\aab",
...
- `scripts/Register-DashboardTask.ps1`: **16 Sep 2026** — execution time limit raised from 5 to 60 minutes and `-MultipleInstances IgnoreNew`. Found on Dan's PC: the task showed Ready, Last Run Result `0xC000013A`, and the dashboard went stale, because a full scan had grown past 5 minutes and Task Scheduler killed every run before it wrote anything. Re-register once (`Register-DashboardTask.ps1`) or set the limit by hand in Task Scheduler.
- `scripts/Deploy-Dashboard.ps1` (19 Sep 2026): publishes every `.html` in `tools\served\` to `<deployPath>\..\tools` (or `toolsDeployPath`), as-is, for the reporting tools to be opened from an address on an iPad (REPORT-LOAD-LATEST-TOOL-HANDOFF.md). No files there yet.
- `scripts/Update-Dashboard.ps1`: **v2.66** (26 Sep 2026: AAB state `actioned`. The rig never closes: it acknowledges (each crew's TSL), then reports the requested action done with evidence (`action: 'actioned'`, rig page Rev 5), and Technical Services review and close from the fleet compliance page (`action: 'close'`, `role` `Technical Services`, posted by `aab-register.js`'s close form on `register.html`). `aabStatus[]` rows carry `actionedAt` / `actionedBy`; `actioned` is not overdue. Rig page Rev 4 made the role a dropdown of the rig crafts; Rev 3 put the originator on the acknowledgement record so the AAB Notifications flow mails the originator and the rig's four. The reports dashboard's AABs tab is the master AAB register (Priority 3 only), rigs mirrored as state chips, click a chip for who is left.)
- `dashboard/dashboard.html` 26 Sep 2026 (no scanner change): the full-report viewer renders SSORT 148's `calcData` (ram cavity dimensional inspection, rolling handoff entry 36) as posted, never judged; per-cavity `status` and `checks[]` shown when the tool starts posting them. Rolling handoff entries 33 to 36 answered.
- `scripts/Update-Dashboard.ps1`: **v2.65** (24 Sep 2026, later: the AAB loop's dashboard side, `AAB-LOOP-PLAN.md` §4 against Eric's schema 2.0 handoff. `seadrill-aab_*` / `meta.kind` `aab` posts and `seadrill-aab-ack_*` / `aab-ack` acknowledgements are read into `aabRecords[]`, `aabAcks[]` and `aabStatus[]` (one row per current AAB per applicable rig: outstanding, partly acknowledged, acknowledged, action open, closed, withdrawn, overdue overlay), never into `reports[]`, the digests or the share's `reports` folder; `aab\aab-data.js` carries the full records with attachments, photographs and PDF and is deployed to `<share>\aab` (`aabOutputFile`, `aabDeployPath`); `aab-overdue-pending.json` is the daily chase feed (`aabChaseFile` copies it into a synced library for the flow). Dashboard the same build: an **AABs** tab with the Priority 3 line (counts and the two links to the board; the loop itself lives on the board, Dan 24 Sep) and a card reserved for the corporate master AABs (Priority 1 and 2, source to be agreed). New `aab\\` folder, the Seadrill Bulletin Board: `bulletin-board.html` (open rig page: choose your rig, acknowledge, close with evidence; posts `seadrill-aab-ack_*`), `register.html` (fleet compliance with evidence, behind Eric's gate fragment inlined verbatim) drawn by `aab-register.js` (also for Eric's Rev 9), Eric's gated create page Rev 8, his `set-password.html`, the logo, `acknowledge.html` as a redirect; `Deploy-Dashboard.ps1` publishes it; `Archive-ProblemFiles.ps1` never moves an AAB file. Guides: `AAB-INSTALL-GUIDE.md`, `AAB-NOTIFICATIONS-FLOW-GUIDE.md`; reply `AAB-REV8-REPLY-FOR-ERIC.md`.) v2.64 = 24 Sep 2026: rolling handoff entries 28 to 32. `cbmData.cbmlabels` (SSORT 148) read beside the CBM keys: `cbmGrades[].task` carries NOV's task wording, the heatmap row label, history title and tooltips print it, the viewer prints it after the item number, the Copilot digest prints `wording (grade)` / `wording (note)` (`$DigestSchema` 4). Id-based CBM keys with a letter (`7_1_2B`) or a cavity suffix (`6_1_2_c0`) are now matched by scanner and viewer, both were silently dropped before; the viewer's key prefix now underscores every non-alphanumeric character so split ram block classes (`Ram Block::Shear`) render. The generic `Ram Block` class is counted on the scan line and flagged `replay` under an SSORT 148+ stamp. `oemCopies[]` records `sourceFormat`, `htmlName`, `htmlBytes` for SSORT 148's HTML OEM copies.) v2.63 = 23 Sep 2026, evening: rolling handoff entries 24 and 25. `meta.rev` (WCGRRT REV n; SSORT from REV 148) is carried as `reports[].rev` and `cbmGrades[].rev`; the dashboard shows it after the file name on the report row and as 'Tool build' in the viewer, nothing when absent. A positional CBM key in a post stamped `SSORT REV …` cannot come from a live build (the positional renderer has been shadowed since before REV 147), so it is listed on the Errors button as kind `replay`, 'still shown', never archived; `Archive-ProblemFiles.ps1` leaves it. Proven on a synthetic SSCE Equipment post.) v2.62 = 23 Sep 2026: rolling handoff entry 22. The Copilot digest prints every `equipEntries[].soak` key by its printed label from `soaklabels` (WCGRRT REV 165) under 'Surface tests', and never prints the labels block itself. Dashboard the same build, plus later on 23 Sep for REV 166 (entry 23): the acoustic function test (`acst_*`), the EHBS test (`ehbs_*`, three rig classes, DMAS parts) and the surface drawdown (`dd_*`, rows from `dd_rows`, posted verdicts shown, over-limit times in red) render under the equipment entry with header, functions, ASR, signatures and notes, labels from `soaklabels`; CBM grade colours follow NOV's five-level meaning, 1 and 2 blue tones, 3 monitor orange 'acceptable with findings', 4 and 5 fail red, entry 21.4. v2.61 = 22 Sep 2026: rolling handoff entry 19. Positional `o:` CBM keys dated before SSORT's REV 80 template rebuild (19 Jul 2026) are kept on their own heatmap rows and history lines under `itemKey` `o:<s>.<i>@pre80`, labelled `(template before 19 Jul 2026)`, with `era` and `postedKey` on the row; an optional `cbm-key-map.json` beside config.json re-points known keys to their current index or marks a task removed. Dashboard the same build: a legend line under the heatmap when such rows exist. v2.60 = 19 Sep 2026, later the same day: `Get-Prop` is case-insensitive on dictionaries, exact lookup first and one key scan on a miss, so no future key with a capital letter can read as absent on Windows PowerShell; the reporting-tools session keeps its keys lower case regardless. Dashboard the same build: an amber note on any report entry dated more than a day before the report date, the REV 161 resumed-draft defect the tool fixes in REV 162. v2.59 = 19 Sep 2026: (a) `meta.reportdate` (lower case, WCGRRT REV 161's actual key) read as well as `reportDate`: dictionary lookups on Windows PowerShell 5.1 are case-sensitive, so Brad's 18 and 19 Sep dailies were being filed under their tile date, 17 Sep; (b) a post that replaces a bigger one under the same filename (fewer entries or photos than the previous cached copy) is warned about and listed in `scan-problems.json` as kind `shrunk`, shown on the Errors button under 'Replaced by a smaller post, still shown', never archived; (c) the deploy step moves a report copy it is about to overwrite to `<deployPath>\reports\_replaced\<name>.<posted time>.js`, so no posted version is ever lost; (d) `attachments[]` (REV 161: name, type, note, bytes, data URL) counted on the summary as `attachments`; the dashboard renders an Attachments section with inline images and download links from the report copy. v2.58 = 18 Sep 2026: the report cache. After a report is read once, a copy with every photograph and other long base64 string replaced by a short marker (`data:image/jpeg;base64,CACHED`) is written to `scan-cache\` next to config.json, named `<report>.<8 hex of the path>.json` and keyed by size + last-write ticks + `$CacheSchema`. The next full scan reads that copy instead of the real file, through the unchanged loop, but only while the file's Copilot digest is current (a stale digest needs the real file). OEM copies, precharge requests and files under 200 KB are never cached; a report that changes, moves or is deleted invalidates or removes its copy. `-NoCache` reads every real file for one run. The rule this rests on: **nothing in the per-file loop may need photo bytes**; the dashboard's report copy and the digest come from the real file. The `Timing:` line now shows `N from the cache`, and a `Report cache:` line counts reads, writes and removals. v2.57 = 17 Sep 2026: `scan-lock.json` next to config.json while a full scan runs, holding the process id; a second scan started while the first is still running prints one yellow line and stops, a lock whose process is gone is taken over. Dan's manual `-Force` scan overlapping the task's own scan took 539 s against 340 s alone. v2.56 = the SSCE COC write-back stamps the review copy with the live file's size + mtime and a hash of the approved list, and skips the whole read/parse/serialise/write when the stamp matches: 143 s of every full scan on Dan's PC. The deploy step lists the server's `reports\` folder once and compares in memory instead of Test-Path + Get-Item per file over the share: 89 s. v2.55 = `scan-problems.json` also carries the no-rig-identity posts as kind `unattributed`; `Archive-ProblemFiles.ps1 -IncludeUnattributed` moves them, and on 17 Sep Dan moved eight, including the 74.8 MB vendor audit and a 29.9 MB vendor surveillance, into `C:\TSC-Dashboard\archive\2026-09-17\unattributed\`; v2.54 (digests are not rebuilt for an unchanged source: each digest carries a `source-stamp` meta of size + mtime + `$DigestSchema`, and a match skips the build; on Dan's PC digests were 560 s of a 1,122 s full scan. Bump `$DigestSchema` when the digest rendering changes. `Timing:` now breaks the output phase down: data file, notifications, precharge inbox, COC write-back, planning data, deploy; v2.53 (digest footer no longer carries the scanner version, so a release does not rewrite all 285 digests; `Timing:` lines split a full scan into reading JSON, writing digests, and outputs/deploy. Dan's PC, 16 Sep: a full scan took **1,152 s**, which is why the 5-minute task limit was killing it; v2.52 = prints `Scan finished in N s` at the end, so the run time is visible against the task's limit; v2.51 (16 Sep 2026: `reportDate` = `meta.reportDate`, else the newest tileDate, else meta.date, proven on Brad's four real daily reports; v2.50 = rolling handoff entries 10 and 11: `reports[].reportDate` from the first tile's `tileDate` (daily reports filed under their own day, not the visit start), digests print Date + Visit start, reports sorted by it; `pdcData` shares the 40 MB ceiling. Dashboard same day: Dates column shows report date with visit span and posted time underneath, viewer renders `equipEntries[].soak` (function test + EDS tables) and `pdcData` (per-BOP fields, rows, photo sections), print keep-together rules; v2.49 = `oem-copy` payloads (CBM-to-OEM loop, `CBM-OEM-HANDOFF.md`) recorded as `oemCopies[]` and hidden from every report path; dashboard shows a Sent-to-OEM chip on the CBM row; v2.48 = rolling handoff entry 9: CBM ceiling 30 → **40 MB** to match SSORT REV 146, both tools now at photo quality 0.82; v2.47 = rolling handoff entry 7: the two potable-water flush observation items (SSORT REV 145) get "No observation recorded (ticked only)" in the digest comment cell when ticked with an empty comment; the dashboard inverts the attention rule for those two items only, see INTEGRATION-CONTRACT; v2.46 = `scan-problems.json` written next to `reports-data.js` on every full scan, the Errors list with full paths, consumed by the new `scripts/Archive-ProblemFiles.ps1`, which lists the unusable posts, asks for a Y, moves them to `<repo>\archive\<date>\<kind>\` outside every report folder and logs each move in `ARCHIVED.log`; oversized reports are never moved, there is no switch for it (Dan, 14 Sep). Dashboard the same day, Dan's sweep: **Marine Integrity tab removed** (scanner still emits `marineScores[]`, the viewer still renders `marineData` inside an old report), the red problems banner replaced by an **Errors (n)** pill at the right of the tab strip that opens a modal listing not-on-dashboard files and oversized reports separately, and the Copilot button restyled gold and renamed **Ask SACRED AI**; v2.45 = the nothing-changed short cut — a SHA-256 fingerprint of every scanned file's name/size/mtime + config.json text + version + today's date is recorded in `scan-state.json` next to config.json after a run in which every deploy step succeeded; the next run with an identical fingerprint only refreshes `generatedAt` in the three data files and their deployed copies and exits in about a second; any changed input, a new day, a missing deployed file, a failed deploy or `-Force` runs the full scan unchanged; digests now carry explicit `Critical items (0)` / `Actions raised during the visit (0)` 'None recorded' lines so Copilot stops improvising actions from narrative; v2.44 = report digests for Copilot Studio: optional `digestPath`/`dashboardUrl`/`copilotUrl` in config.json, one HTML digest per report into a separate synced library, scanner sole writer, see `COPILOT-REPORTS-ASSISTANT-GUIDE.md`; v2.42 = `_psi`/`_tp`/`_dp` merged as reading companions, no more phantom readings; v2.41 = rolling handoff entry 6: the size
  ceiling is applied per report type once the file is parsed — 10 MB for
  everything, **30 MB for CBM Inspection** (40 MB from v2.48), because a CBM report legitimately
