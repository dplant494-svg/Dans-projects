...
**Scanner v2.56 (17 Sep 2026): the review copy is only rewritten when something changed.** The scanner puts an HTML comment straight after `<head>` in the review copy, `<!-- coc-writeback-stamp: <live file size>|<live file last-write ticks>|<hash of the approved list>|w1 -->`, and on the next run reads just the first 4 KB of the review copy: if the stamp matches the live file and the same approved requests (requestId, asset, oem, serial, siteUnit), it prints "review copy already current" and touches nothing. Any change to the live file, any new approval, or a missing review copy makes it rebuild from the live file as before. Reading, re-parsing and re-serialising the 1.9 MB dashboard from the SSORT share took 143 s of every full scan. Whoever maintains the COC dashboard: the stamp is on the review copy only, never the live file, and nothing needs to be done with it.
